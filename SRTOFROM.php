 ...
 <div class="wrapper">
    <div class="container mt-5">
      ...
      <div class="form-group">
        <input style="padding: 5px"  id="OrderProduct" onkeyup="SearchByProduct()"></input>
  ...
  var ajax = new XMLHttpRequest();
      var method = "Get";
      var url = "StockReport-DisplayDataToFrom.php?from="+from+"&to="+to;//Invoice == Report
      var asyn = true;
      ...
      ajax.onreadystatechange = function ReportDisplay()
      {
        if(this.readyState == 4 && this.status == 200)
        {
          ...
          {
            var Invoice = data[i].sr;
            var ProductName = data[i].ProductName;    //InvoiceNo,CustomerAddress,Amount,Date,Paid
            var OrderQuantity = data[i].OrderQuantity;
            var DDY = data[i].Date;
            d += "<tr data-id='"+Invoice+"'>";
              d +="<td data-target='Invoice' class='CustomerName'><a href='InvoiceReport.php?id="+Invoice+"'>"+ Invoice +" </a></td>";
              d +="<td data-target='CustomerName' id='CustomerName' class='CustomerPhone' >"+ ProductName + "</td>";
              d +="<td data-target='DDY' id='DDY' class='CustomerAddress' >"+ OrderQuantity + "</td>";
      ...
      function SearchByProduct() 
    {
      ...
      input = document.getElementById("OrderProduct");
