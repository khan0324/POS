...
<body>
    ...
    <div class="container mt-5">
      <h4 class="T main_bg_color">PAID RECEIPTS</h4>
      ...
      <table style="" class="table table-bordered table-hover" id="tabledata">
        <thead class="" style="color: white">
          <tr class="main_bg_color">
            ...
            <th scope="col" class="in" style="width: 15%">Vendor Name</th>
            <th scope="col" class="in" style="width: 15%">Date</th>
            <th scope="col" class="in" style="width: 15%">Paid Amount</th>
          </tr>
        </thead>
        <tbody id="ReportTable" style="text-align: center;background-color: white">
  ...
  <script type="text/javascript">
    //Display Function For Customer
      ...
      var method = "Get";
      var url = "PaidReceipt-DisplayData.php";//Invoice == Report
      var asyn = true;
      ...
      ajax.onreadystatechange = function ReportDisplay()
      {
        if(this.readyState == 4 && this.status == 200)
        {
          ...
          {
            var sr = data[i].sr;
            var CustomerName = data[i].vendorname;    //InvoiceNo,CustomerAddress,Amount,Date,Paid
            var DDY = data[i].dat;
            var PaidAmount = data[i].PaidAmount;
            d += "<tr data-id='"+sr+"'>";
              d +="<td data-target='Invoice' class='CustomerName'><a href='PrintPaidReceipt.php?id="+sr+"'>"+ sr +" </a></td>";
